    ...
    return (
        <div className='p-5 rounded-md shadow-xl bg-white border border-gray-100 "bg-white dark:bg-gray-900 dark:text-white transition-colors duration-300'>
            <div className='flex items-center justify-between'>
                <p className='text-gray-900 dark:text-gray-100'>{daysAgoFunction(job?.createdAt) === 0 ? "Today" : `${daysAgoFunction(job?.createdAt)} days ago`}</p>
                <Button variant='outline' className="rounded-full" size='icon'><Bookmark /></Button>
            </div>

            <div className='flex items-center gap-2 my-2'>
                <Button variant="outline" size="icon" className="p-0 rounded-full">
                    <Avatar className="w-10 h-10"> {/* Set width & height */}
                        <AvatarImage src={job?.company?.logo} />
                    </Avatar>
                </Button>
                <div>
                    <h1 className='font-md text-lg'>{job?.company?.name}</h1>
                    <p className='text-gray-900 dark:text-gray-100'>Nepal</p>
                </div>
            </div>
            <div>
            ...
            <div className='flex items-center gap-2 mt-4'>
                <Badge className="text-blue-700 font-bold" variant="ghost">{job?.positions}Postions</Badge>
